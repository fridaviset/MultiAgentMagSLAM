...
for i=1:3
subplot(1,4,i);
plot(x_DR(1,:,i),x_DR(2,:,i),'r','linewidth',1.2);
hold on;
plot(x(1,:,i),x(2,:,i),'b','linewidth',1.2);
axis equal;
xlim([xl+0.7*marg xu-0.7*marg]);
ylim([yl+0.7*marg yu-0.7*marg]);
box off;
xlabel('x','Interpreter','Latex');
ylabel('y','Interpreter','Latex');
set(gca,'fontsize',fontsize);
set(gca,'TickLabelInterpreter','latex');
exportgraphics(gca,['OdometryAgent',num2str(i),'.png'],'Resolution',500);
end

subplot(1,4,4);
plot(1,NaN,'b','linewidth',1.2); hold on;
plot(1,NaN,'r','linewidth',1.2);
legend('Ground truth','Odometry','Location','North');
set(gca, 'FontName', 'Times');
box off;
axis off;
set(gca,'fontsize',fontsize);
exportgraphics(gca,'OdometryIllustrationLegends.png','Resolution',500);
...
for i=1:n
    ...
    subplot(n+1,1,i);
    errors_multi(:,i)=reshape(sqrt(sum((x(:,1:N,i)-x_hat(:,:,i)).^2,1)),N,1);
    p%lot(time,errors_multi(:,i),'Color',black,'linewidth',1.2);
    hold on;
    errors_single(:,i)=reshape(sqrt(sum((x(:,1:N,i)-x_hat_single_agents(:,:,i)).^2,1)),N,1);
    %plot(time,errors_single(:,i),'Color',green,'linewidth',1.2);
    hold on;
...
set(gca,'TickLabelInterpreter','latex');
grid on;
exportgraphics(gca,['Illustrations/OdometryErrors',num2str(i),'.png'],'Resolution',500);
end
subplot(n+1,1,n+1);
%plot(1,NaN,'Color',black,'linewidth',1.2); hold on;
%plot(1,NaN,'Color',green,'linewidth',1.2);
...
axis off;
set(gca,'fontsize',fontsize);
exportgraphics(gca,'Illustrations/OdometrynErrorsLegends.png','Resolution',500);
